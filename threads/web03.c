...
#define GET_CMD     "GET %s HTTP/1.j0\r\n\r\n"

int     nconn, nfiles, nlefttoconn, nlefttoread;

int             ndone;      /* number of terminated thread */
pthread_mutex_t ndone_mutext = PTHREAD_MUTEX_INITIALIZER;
...
int main(int argc, char **argv)
{
    ...
    while (nlefttoread > 0) {
      while (nconn < maxnconn && nlefttoconn > 0) {
        ...
        Pthread_mutext_lock(&ndone_mutext);
        while (ndone == 0)
