...
typedef struct {
  void      *dg_data;       /* ptr to actual program */
...
dg_echo(int sockfd_arg, SA *pcliaddr, socklen_t clilen_arg)
{
  int       i;
  const int on = 1;
  sigset_t  zeromast, newmask, oldmast;
  ...
  /*
   * Sigprocmask() is used to fetch and/orchange the signal mask of
   * the calling thread. The signal mask is the set of signals whose
   * delivery is currently blocked for the caller.
   * The behavior of the call is dependent on the first argument, as
   * follows.
   *    SIG_BLOCK
   *        The set of blocked signals is the union of the current set
   *        and the second argument.
   *    SIG_UNBLOCK
   *        The signals in second argument are removed from the current
   *        set of blocked signal. It is permissible to attempt to
   *        unblock a signal that is not blocked.
   *    SIG_SETMASK
   *        The set of blocked signals is set to the second argument.
   * */
  Sigprocmask(SIG_BLOCK, &newmask, &oldmask);
  for ( ; ; ) {
    while (nqueue == 0)
      sigsuspend(&zeromask);    /* wait for datagram and proccesing by handler. */
...
sigio(int signo)
{
  ...
  cnread[nread]++;      /* histogram of # datagrams read per signal */
}
